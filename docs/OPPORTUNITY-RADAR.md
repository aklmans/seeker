...
网络护栏，并持久化 `sourceVerified/sourceVerifiedAt`。候选入库时先写入未激活的 Rust 私有
验链凭据；只有报告写入与校验均成功后，runner 才在提交 run 最终成功的同一事务中激活凭据。
该凭据不在通用集合白名单、不进入便携备份，指纹绑定完整候选内容、稳定去重键、task 和 run。
因此改写公开 run/step JSON 不能把失败运行伪造成可信来源；普通机会 CRUD 与便携导入同样会
强制降级为未验证，必须重跑雷达验链后才能接受。
列表读取和“接受为正式岗位”复用同一可信判定：整次 run 与 `verify_sources` 步骤都为
`succeeded` 且私有凭据仍匹配时才显示已验证；运行中、失败或中断的结果一律降级。
固定招聘页会保留安全的绝对与相对岗位 `href`；发生受控重定向时，相对链接只能以最终响应
页面 URL 为基址解析，不能沿用用户最初提交的 URL。
...
不宣称它在技术上可验证为只读。Web 没有该私有授权表，导入、存储、读取和再次导出时都会
剥离 JSON 自报的授权派生字段。模型规范化阶段结构性无工具。
