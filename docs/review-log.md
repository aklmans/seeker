...
### ★★ 第88轮独立复核 = 简历解析 Cut2 🏁 承重写通过 + 1 [建议](merge 抽测,已落 `ab5b205`)· jobseek mock 面全线清零
**评审读码核实承重 SKILLS 写受 schema 硬闸保护(失败一字节不写)、信任分层完整、Cut1 [建议] 已落。**
- **★承重保护(最关键)读码坐实**:schema 硬闸失败/无技能→诚实降级 return、`applyParsedResume` 不执行 ⇒ **SKILLS 逐字节不变**;畸形 AI 输出绝不污染承重(ivSubmit 第78轮纪律延续)。信任分层(untrusted:text / instruction 纯常量 / ai_generate 无工具)· Cut1 [建议] lvl 1-5 已落(stored===effective)· merge 保 demand/pri 新技能诚实默认(非编造)· resume 定清单与 AI-only 无手工录入设计一致故**无手工编辑可 clobber**。
...
### Skills 完整版 F1:工具 scoping 运行时(契约 + 三态收窄,无 UI)· commit `50eef98` · ⏳ 待审
承第89轮方案设计过审 + 用户拍板起 F1。兑现第89轮 F1 盯点①②③④(逐条):
- **①两道结构闸真 enforce(真模块导出验)**:**gate A 工具表** = 新增 `scopeAppTools(readableTools, scopeTools)`([readable.js](web/platform/capability/app-tools/readable.js) 纯函数、node 可测、真模块导出)把 readable 描述符按声明取子集,streamReply `appTools:scopeAppTools(readableAppTools(), scopeTools)` → 非声明 app-tool 不进 rt.ai.stream。**gate B dispatch** = ai.rs:815 `app_tool_names.contains(call.name)` 从**同一** app_tools 派生(ai.rs:744)⇒ 前端收窄 app_tools 后工具表与 dispatch 白名单**同源同步收窄**,幻调声明外工具不匹配、不执行(Rust 侧已测)。**平台 Rust 能力**(query_data/memory/show_widget/doc)由 ai.rs:720 `registry.tool_schemas()` 恒加、与前端 app_tools 无关(实测坐实,744 才叠加 app-tool)⇒ scoping 不动平台能力、恒在。
- **②三态逐条测 + 变异证红**:[test/app-tool-scope.test.mjs](test/app-tool-scope.test.mjs) —— ①undefined=全 readable ②[]=空 ③['x']=仅 x∩readable + 减权子集(声明含 profile_dump/secrets_read 诱饵→丢弃、⊆readable)+ D3 叠加(filterReadableTools 拦下的 secret_tool,Skill 声明也捞不回)。**变异证红**:去 undefined 守卫→三态①红 / undefined→[]→三态①红 / 收窄分支忽略 scope→三态②③+减权子集+D3 叠加红;还原 6/6 绿。normSkill tools 三态保真单测(undefined≠[] `notDeepEqual` 承重)+ 畸形归一(非数组→undefined 不塌成 [])。
- **③红线继承(scoped⊆用户打字、无旁路)· 活线 E2E**:runSkill→`agentSend(prompt,undefined,s.tools)`→streamReply→scopeAppTools 直穿(normSkill 保三态)。**preview 活线**(stub `__TAURI__` 使 aiChatAvailable 真、捕获 SeekerRT.ai.stream 的 appTools,同 URL import 真 runSkill):undefined→[mv] / []→[] / [mv]→[mv] / [不存在]→[](声明外够不到)。scoped Skill 仍走用户打字同路径(工具**更少**、⊆),D3/profile/guardrail/设置不可经对话改 全照常继承。
- **④`[]`=「无 app-tool」非「无工具」固化进类型**:[types.d.ts](web/platform/shell/types.d.ts) `Skill.tools?` + [skill-model.js](web/platform/shell/skill-model.js) `NormSkill.tools` 三态注释(平台 Rust 能力恒在、各由自身红线闸;「纯文本无数据」是另一机制非 `[]`)。
...
### ★ 第91轮独立复核 = Skills 完整版 F2(管理 UI)🏁 通过 + 1 [建议](修 proposal 反转安全语义残留,已落 `861541d`)
**评审逐条核实三态 UI 映射正确(undefined≠[] 保真、不 clobber、往返保真)、cEsc、管理不经对话、声明⊇生效与 F1 一致、无 UI 旁路;+ 1 重要 [建议](proposal 内部矛盾=反转安全语义地雷)。**
- **盯点核实**:①三态 UI(allTools.length===0→不 clobber 保留 s.tools / 未勾→undefined 全零回归 / 勾+空→[]无 app-tool / 勾+['x']→['x'];**undefined≠[] UI 可辨 + 往返保真 preview 证 = F1 承重**)②cEsc(value+label)+管理不经对话(§4-2)③数据层零改+不 clobber④**声明⊇生效与 F1 一致**(UI 列 appTools() 全集=声明意图、运行时 scopeAppTools ∩ readable 减权=生效)⑤**无 UI 旁路**(声明不可读工具→运行时丢弃無效;列 appTools() 是平台读契约 registry/SeekerShell **非 import app** §1)⑥`[]` 语义 UI 固化(hint 平台能力始终可用=第89轮 [建议]④)。runSkill 仍 `agentSend(s.prompt,undefined,s.tools)` F1 穿线未破。npm 79/0 亲跑。
...
### Scheduled tasks 方案(定时跑 Skill · 无人值守红线)· commit `022d907` · ⏳ 待审(绿地设计 · 方案门控)
承第94轮次序(绿地方案先行)+ 用户拍板 **Scheduled 先于 Project**(Project=目标工作区语义已对齐、后续单出)。
- **★先量六条(§1)**:①Rust 零 scheduler + ai_chat 仅前端命令(后台自发起=全新 plumbing)⇒ **调度器落前端壳、fire = runSkill 既有结构 ⇒ 零新 Rust 面、红线结构性继承**(四红线+F1 scoping+I1 needsReview 双点拒,不是重新实现)②`auto_backup_if_due` 事件驱动 due-check 先例③Skills 全线可用 ⇒ 守卫免费继承④单全局线程 ⇒ 产出落同一 Agent 对话⑤streaming 闭包局部 ⇒ 需一枚壳级忙信号⑥`platform_schedules` 三处 + **DB_VERSION 3→4**(S1b 教训预防)。
- **★红线(设计核心=无人值守)**:①运行授权语义 = 用户预先发起的重放(fire 经 runSkill=用户打字路径,红线照常)②**破坏性 = 结构性 fail-closed 已有**:模型只能提议、无人点=不执行、确认卡留对话等用户回来;**「预授权确认」结构上不存在、绝不引入**(否则拆第58轮「用户显式点击不可伪造」闸)③**★★AI 不能给自己排任务(唯一新增红线)**:Agent 能排任务 = 自我持续执行通路(自激励循环+BYO 成本)⇒ platform_schedules **不进 QUERYABLE** + 调度 CRUD **只在管理面**(§4-2 延伸)④未审阅导入件不可被调度(**双点**:选择器只列已审可运行 + runSkill 守卫兜底——即便调度指向后来变待审的 Skill〔如 [建议]2 编辑重审〕fire 也 no-op)⑤BYO 成本可见(last_run 记录+一键停+**错过不补跑防开机风暴**)。
- **设计**:due 判定 = 零 import 纯函数 `scheduleDue`(node 可测;错过多个排点只 due 一次)+ normSchedule fail-safe;调度器 = 壳 boot setInterval(60s) tick,忙则跳过本 tick;管理 UI = 能力中心段(同 Skills 形制);文案说清「仅 Seeker 开着时触发」。
- **分期**:SC1 = 契约+调度核心+最小管理面**一刀闭环**(F1 教训:无消费者机制=休眠件)/ SC2 = 打磨(运行记录/错过提示)。
...
### ★ 第98轮独立复核 = Project 方案 🏁 设计通过 + 1 [应改](已订)+ 1 [建议](已入拍板项)
**评审坐实死功能翻案、refute 我的「Rust 零改」、识别认可新红线;[应改]/[建议] 全落 proposal `43fed7a`。**
- **①死功能翻案坐实(评审独立验:前端全仓零 sessionId 传点)**:「多轮历史 #1 G2」自实现起从未生效 —— **一整个设计好、写了代码、带封顶、却从没接通的功能**。**第六次先量翻案,这次翻的是存量设计的生效假设。**
- **★[应改](我独立核实三重职责后已订方案)**:我只看到 sessionId 的 History-bucket 职责;它还承 **(a)流事件路由**(desktop.js:65 `hit` 认领 chunk/done/error)**(b)取消键**(Sessions.insert **同 key 顶掉第一条流的令牌**、ai_cancel 歧义)⇒ 直接稳定化 = **并发流相撞**(定时 fire 在飞 + 用户打字〔agentSend 无忙闸〕同 key 两流 ⇒ 事件交叉投递 + 令牌 clobber;**今天并发正常恰因 id 每次 fresh —— 稳定化拆掉的正是这个**)。**修 = 拆键**:session_id 每流 fresh + ai_chat 可选 `historyKey` 键控 History(缺省 fresh=现行为,Rust ~5-10 行)。「隔离论证(History 按 key 隔离)仍成立,key 须专用参数」。**★方法论:先量要量全一个符号的所有职责,不是量到第一个够用的就停。**
- **③新红线「项目管理不经对话 = 自我提示注入通路缺席」识别正确(评审:比自排任务更直接 —— 那是自我持续执行,这是自我改写行为基线)**;三件套+caps.len 写半落点认可;∉QUERYABLE 理由对(当前项目指令注入=功能有界;query_data 枚举所有项目=跨项目泄漏违隔离承诺)。
- **④通路 A + task 纪律正确,附三条钉死(已入 PJ3 判据)**:①来源钉死(projectInstructions 只来自管理面用户自撰、永不含模型/RAG/外部派生 —— system 邻位=高权位,固化类型注释同 greeting 第50轮)②不入 history ③契约扩展必审。
...
### ★ 第101轮独立复核 = Project PJ3 🏁 通过无 findings · ★★Project 线收官 + 能力中心八段全真
**评审读码坐实注入位分层、三条钉死全结构性、连带语义与四态覆盖;两条线同时闭合。**
- **①注入位读码坐实(最终序)**:`[0]system(平台基线)→[1]项目指令(可信,框定「用户本人在管理面自撰、以用户身份生效」)→[2..]history→[n-1]context(记忆/RAG 召回,Untrusted 框定)→[n]user` —— **可信政策贴 system 高位、不可信数据贴 user 低位带框定 = 正确的信任分层**;contribute 零改动。None/空白零插 + 变异红。
- **②三条钉死全结构性(非约定)**:①来源=唯一赋值点三闸 + 类型钉「永不派生」②**不入 History = `append_turn(key, user, assistant)` 类型签名上就进不去**(比「记得别存」强一档)③契约扩展=送审主体、编译器证唯一调用点。
- **③连带语义对**(显式 historyKey 不注入;E2E 四态覆盖全部分支)· **④来源框定文案恰当**:「以用户身份生效」是 **frame_untrusted 的信任镜像** —— 对模型断言用户权威,而这**由构造为真**(管理面唯一写入+类型钉死);框定与事实一致、不过度声称 · **⑤`<注入试探>` 原样穿参认可**:用户自撰=可信侧(对自己的 agent 下指令=与打字同权);注入位纯文本 content 无 DOM 面(指令进 DOM 唯一处=PJ1 管理面已 cEsc);**「防注入」防的是第三方内容冒充指令,不是用户指令自己**。
- **验**:cargo 134/134 · npm 102/0 亲跑 · tsc 51 · 真机 0 panic · 文案欠账结清。
- **★★收官盘点(评审)**:**Project 线**(方案[死功能翻案+拆键 [应改]]→PJ1[守卫三件套+fail-safe 选边]→PJ2[拆键+隔离+G2 修活]→PJ3[注入+三条钉死])每刀盯点全兑现、[应改] 一条当轮订正。**能力中心八段全真**:Connector(MCP)· 工具(app-tool T0-T3)· 记忆 · 知识库 · Skills(S1-S4+F1/F2 scoping+I1/I2 导入分享)· Scheduled(SC1/SC2)· Project(PJ1-PJ3)——**没有占位、没有 mock、没有「规划中」**。回望第51轮 AI-Native 转向方案的诊断(两个重叠面板/8 处演出 mock/201 行孤立薄页):**窗口收敛了、mock 清零了、assets 退役了、能力层从 4 个 Rust 能力长成完整的用户能力体系 —— 且每一步红线(D3/profile/guardrail/设置不可经对话改/QUERYABLE 静态底/信任分层/无人值守只提议)都以结构或 CI 承接,未破一条。**
- **★下一步(评审建议,按序)**:①**真机 BYO 端到端冒烟一趟(最高杠杆)** —— 历轮「待 BYO」诚实边界已积累成批(多轮 prior 真注入 PJ2/项目指令真生效 PJ3/app-tool 真调用 T2-T3/scheduled fire 真流 SC1/导入 Skill 审阅后真运行 I1/简历解析·出题·改写·反馈真模型 块i),一次桌面+BYO 会话成批关掉;**回归用户「真机反馈驱动」的原点,发现什么按什么定下一刀**②旧债盘点(R1 GUI 冒烟/10d② 桌面 persist 写路径/profile 批8/#6 签名公证[用户 Apple 证书=手动])③已记后续单出(history 跨重启重建/per-project 资料域/Skills trigger/guardrail onConfirm 余项)。

### ★★ 真机反馈:前端热嵌 bug —— 「新功能一个都看不到」根因 + 修复 + 验证协议认账 · commit `e971302`
**用户真机截图:侧栏功能点不出效果、UI 停在旧形态。这是本会话最重要的一条反馈,暴露我历轮「真机金标准」的系统性盲区。**
- **根因(可靠信号,非 strings)**:`target/debug/build/*/output` 里 `rerun-if-changed` 只追踪了 `../web/domain/prompts/prompts.json` **一个文件** —— `index.html`/`platform/**`/`apps/**` 真前端**完全没被追踪**。机理:Tauri `generate_context!` **编译期**把 frontendDist 嵌进二进制,但 **stable Rust**(rustc 1.95)上无法用 `proc_macro::tracked_path`(nightly-only)告诉 cargo「资产变了要重编译」;本项目又无 `devUrl`、一直走嵌入资产 ⇒ **只改前端文件时 cargo 认 src-tauri 输入没变、复用旧 crate 产物、`generate_context!` 不重跑 ⇒ 二进制里是旧前端**。
- **★两个实例的混淆(现场诊断)**:`open_application "Seeker"` 被 macOS 解析成用户的 **release `.app`(Jul-8 打包)** 而非我的 debug 二进制;用户双击的正是这个 4 天前的旧包(本会话所有功能之前)。故「一个都看不到」=用户在跑旧前端。
- **★strings 诊断是错的(自我订正)**:我先用 `strings` 找前端特征串全 0、误判「二进制里嵌的不是当前前端」。实则 **Tauri 压缩嵌入资产**,任何前端串(新旧都一样)都不出现在 strings 里(`platform_projects` 出现只因它是 Rust 源码串)。改用 `rerun-if-changed` 追踪清单才是可靠信号。**教训:诊断信号本身要先证伪(查一个「旧前端也一定有」的串——全 0 就证明信号无效)。**
- **修法(build.rs 两步)**:①递归 `rerun-if-changed`(../web 全树 124 文件+目录)②前端指纹(路径+mtime FNV-1a)经 `rustc-env SEEKER_WEB_FP` 暴露、lib.rs `env!` 依赖 ⇒ 指纹变→lib.rs 重编译→`generate_context!` 重嵌;未变则零开销。**★决定性对照验证**:不改→no-op 0.5s / touch web/index.html→**Compiling app** 11s(重嵌)/ 再不改→no-op 0.2s。确定性机制、非推断。
- **★★验证协议认账(系统性盲区)**:历轮「真机金标准」只查**进程存活(0 panic)**——但 (a) JS 错误不杀 Tauri 进程,(b) `cargo run` 复用旧二进制不重嵌 ⇒ **每次都在跑同一冻结旧前端却报「boot 0 panic」**;E2E 又**全走 nocache server 读实时 web/、从没走真二进制**。两盲区叠加,让「新功能进不了真机」漏了整整一个 arc。**修复后验证协议(已入 reviewer-onboarding §5)**:真机验证须 = 改前端后确认 `Compiling app` 重编译 + 截真窗口 DOM/视觉,**不只进程存活**;用户跑 .app 则须 `tauri build` 重打包(dev 的 build.rs 修复同样覆盖 release 打包)。
- **善后**:release bundle 重打包中(`npm run build:all`,用修复后代码嵌当前前端),产出 fresh Seeker.app 给用户双击。
