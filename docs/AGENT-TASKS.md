...
限制但不能超过平台硬上限。机会雷达为每个 run 建立 Rust 私有调用账本，在外部来源或模型请求
发出前原子预占：来源调用上限由检索与逐条验链共同消耗，模型调用上限不会因暂停、继续或崩溃
恢复而重置。账本缺失时执行器 fail closed，不执行不可计量的请求。取消在当前原子操作的安全
边界生效；若无法确定副作用结果，步骤进入 outcome_unknown。
