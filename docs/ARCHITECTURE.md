...
不会删除此副本。卡片、导图、对比表、时间线新建时取当前默认值，已存作品保持独立。聊天 Widget 在未指定
个人默认值时保存 `style.theme=auto`，显式指定的个人默认值仍优先；旧记录缺少该字段时保持原来的独立样式。
该标志只影响 Widget 显示，不扩展集合、备份白名单、AI 查询范围或设置权限。设置中的“应用默认”恢复
新 Widget 跟随应用、其他作品使用极简；作品页也可以单独切换 Widget 的跟随/独立风格。
“保存到我的样式”仅接受独立风格；跟随主题时禁用入口并说明原因，保存入口与默认偏好写入都拒绝 `theme=auto`。
旧版误存的跟随主题收藏保留原记录，但不能复用或设为个人默认；旧的具名 auto 默认按“应用默认”读取，
不自动改写偏好或备份。用户可以选择独立风格后重新收藏。
个人样式列表把便携导入记录视为不可信数据：先验证作品字段、样式对象、版本元数据和历史，再允许应用、
改名或设置默认值；单条异常只显示为不可用的样式，不中断整页。损坏记录仍可移除并通过原有 toastUndo 撤销。
正常样式继续走版本事务；损坏样式经 `rt.creations.setStyleDeleted(expected, deleted)` 在 Web IndexedDB /
Rust SQLite 单事务中比较整份原始 JSON，仅修改 `deleted` 标记。缺少版本也能处理，不补造颜色、内容或历史；
删除及撤销遇到新记录或写失败均拒绝，不覆盖后来修复的数据。这个入口只作用于已有 `kind=style`，不扩展
通用集合、便携白名单、AI 查询或模型权限。导入仍保全原始异常记录，读取侧隔离与可撤销清理负责恢复可用性。
...
`src-tauri/src/widget-design.md` 直接编入 show_widget 的 HTML 参数说明，给模型提供实际可用的
元素、布局类、颜色 token、键盘交互和原生绘图约束；AI 编辑入口采用相同规则。规则指导生成，
不构成任意 HTML 的完整样式/脚本验证器，也不自动改写旧作品。模型仍可能生成错误脚本。

`capability/widgets/design-system.js` 提供语义元素底座和 `sw-*` 布局/控件类；默认继承壳的字体、
边框和背景，图表另用四组明暗配色。跟随模式通过每个 iframe 的专属 MessagePort 热更新，发出
`seeker:themechange` 与 `seeker:resize` 事件供 Canvas 重绘；SVG 直接响应颜色变量。切主题不重载 iframe，
切换到独立作品风格才重新渲染。端口仍只接受父窗口握手，重载会关闭旧端口并拒绝未完成的导出请求。
共享 DOM 移除监听在卡片或祖先容器移除后关闭端口、清空消息处理器与快照入口，并拒绝通道中未完成的导出；
该清理同时覆盖跟随主题和独立风格，不依赖下一次切主题。最后一个端口关闭后停止对应的移除/主题监听，
新卡片建立通道时再启动。

快照先保全表单和 Canvas，再固定 SVG 元素的计算后颜色与字体，避免独立 SVG 图像丢失外部 CSS 变量。
标准 range 用无脚本静态轨道和滑块重建；快照最后覆盖当前根变量，避免样式顺序恢复旧主题。捕获内容
仍经过原有清理器和独立 opaque 导出 iframe。离线交互 HTML 固定导出时的主题快照，不连接父应用。
