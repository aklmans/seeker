...
| 3 | 求职逐页迁入 `apps/jobseek/`(每页一 commit;剩余交织按下方第8轮裁定) | **搬迁收官**(择取+抽壳序1–5 全落,index.html −71%→1330;1–22 轮全过审;8 个 `SeekerShell.*` 契约;余 3.y 类型化[单列里程碑]+ 归属债[settingsPersistOn 系/壳杂项/文案]) |
| 4 | 第二应用「数据资产管理」(验证新增应用成本) | **⚠前置(第22轮[建议]·红线级)**:先或同刀做 `clearAllDataFlow`→`SeekerShell.collections()`(替 `CLEARABLE_COLLS` 硬编码;否则「清空全部数据」静默漏第二应用集合,破坏§4-3 可撤销红线) |
