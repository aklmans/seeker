...
产品当前发布版本为 v0.4.0，包含“岗位投递包”“机会雷达”和“资料整理助手”三条固定任务流。用户批准的 `docs/ROUND-1-PLAN.md` 五批功能已完成本机工程验收，证据见 `docs/ROUND-1-PROGRESS.md`。普通笔记保存、文件解析与任务快照创建不调用 AI；问答和资料整理仅发送明确选择的正文，检查引用并验证真实报告文件。桌面版仍为自带模型，Web 仅提供实际实现的能力。真实模型质量、Windows 真机和目标用户试用未覆盖，详见 `docs/ROUND-1-ACCEPTANCE.md`。新增范围先讨论，不自行扩大。
