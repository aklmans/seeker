...
**探索者 · Seeker** —— 本地优先的日常 AI 助手。首页提问、文字工具、文件阅读和资料库服务普通办公场景；任务中心负责可恢复、可验证的报告生成，能力中心管理连接器（MCP）、工具、记忆、知识库、Skills、定时任务和项目。业务应用通过 manifest 注册，可按需启用或关闭，数据仍保留在本地。
...
产品已公开发布 v0.2.0；当前 v0.3.0 开发版包含“岗位投递包”“机会雷达”和“资料整理助手”三条固定任务流。用户批准的 `docs/ROUND-1-PLAN.md` 五批功能已完成本机工程验收，证据见 `docs/ROUND-1-PROGRESS.md`。普通笔记保存、文件解析与任务快照创建不调用 AI；问答和资料整理仅发送明确选择的正文，检查引用并验证真实报告文件。桌面内测仍为自带模型，Web 仅提供实际实现的能力。真实模型质量、Windows 真机和目标用户试用未覆盖，详见 `docs/ROUND-1-ACCEPTANCE.md`；尚未打包、推送或发布本轮功能。新增范围先讨论，不自行扩大。

第二轮已完成本机工程验收，完整范围见 `docs/ROUND-2-PLAN.md`，证据见 `docs/ROUND-2-PROGRESS.md` 和 `docs/ROUND-2-ACCEPTANCE.md`。工作空间、数据设置、可编辑作品、导图、个人样式与导出分享均已实现，追加的作品弹窗调整已通过浏览器和真实桌面验证。求职应用默认关闭，需要用户主动启用，旧数据保留。真实模型质量与 Windows 真机仍未覆盖；未打包、推送或发布第二轮功能。
