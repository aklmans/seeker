...
3. **反焦虑** —— 不用红色警告、不用倒计时施压。破坏性操作(删/清空/覆盖)**分两档**(第56轮裁定;旧写法「一律统一走 guardrail」与 notes/prompts/resumes/jobs 的既有逐条删相矛盾,是条会被代码证伪的声明):
   ...
   - **★★guardrail 的判据是「用户有没有可失去之物」,不是「是不是写操作」**(第66轮裁决 · 显式豁免先例):
     `memory_repair_corrupt` / `doc_repair_corrupt` 归一化 `created_at` 后**不走 guardrail** —— 因为被覆盖的旧值
     **app 内没有任何代码路径能观测到**(列表经 `int_lossy` 早已显示为 `0`;AI 召回的 `SELECT` 不含此列;
     快照读不了它)。修复是**存储向显示收敛**,不是销毁,且幂等。**在安全的那条路上加模态,只会钝化
     「修复优先于销毁」的偏置。** ⚠ **不得援引它作为「可观测的写操作也可跳过 guardrail」的先例。**
   - **★★销毁之前先问:能不能修?**(第66轮 [建议]强)schema 自己定义了默认值的列可**忠实归一化**;
     没有默认值的列**一个字节都不许碰**,销毁才是它们的逃生口。**决策点要说清「失去什么」,不只是「能不能撤销」** ——
     同一个「不可快照」的行,可能内容仍在被 AI 检索(删 = 永久失去)、或它的损坏正让整个召回哑火(删 = **恢复**检索)。
     **这三种情形必须由后端实测(oracle)得出,不靠推理。**
