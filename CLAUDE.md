...
**探索者 · Seeker** —— 本地优先的个人 AI 工作空间。产品主线是“对话与写作 → 本地资料 → 可编辑作品 → 任务 Agent”：首页和独立会话承接日常思考，资料库管理文件与笔记，作品中心负责继续编辑、版本和导出，需要持续执行的工作进入可恢复、可验证的任务中心。能力中心管理连接器（MCP）、工具、记忆、知识库、Skills、定时任务和项目。求职只是默认关闭的可选业务应用，不是产品定位；业务应用通过 manifest 注册，可按需启用或关闭，数据仍保留在本地。
...
2. **密钥只进系统钥匙串** —— 绝不入库/配置文件/前端/日志;前端只见 `configured/empty`。Web 演示代理的上游 key 只存服务器环境；访问码模式下浏览器只持低价值门票，显式公开模式不向浏览器发任何凭据。
...
8. **应用设计语言统一** —— 暖橙节制(仅句号/标号/CTA/选中/进度)、0.5px 边框、系统字体栈、Mono 大写标签、衬线斜体标题+暖橙句号。用户生成的作品拥有独立可自定义样式（第二轮已批准），作品风格不改变应用壳的视觉体系。
...
产品当前发布版本为 v0.4.1，包含“岗位投递包”“机会雷达”和“资料整理助手”三条固定任务流。用户批准的 `docs/ROUND-1-PLAN.md` 五批功能已完成本机工程验收，证据见 `docs/ROUND-1-PROGRESS.md`。普通笔记保存、文件解析与任务快照创建不调用 AI；问答和资料整理仅发送明确选择的正文，检查引用并验证真实报告文件。桌面版仍为自带模型，Web 仅提供实际实现的能力。真实模型质量、Windows 真机和目标用户试用未覆盖，详见 `docs/ROUND-1-ACCEPTANCE.md`。新增范围先讨论，不自行扩大。

第二轮已完成本机工程验收并纳入 v0.4.0，完整范围见 `docs/ROUND-2-PLAN.md`，证据见 `docs/ROUND-2-PROGRESS.md` 和 `docs/ROUND-2-ACCEPTANCE.md`。工作空间、数据设置、可编辑作品、导图、个人样式与导出分享均已实现，追加的作品弹窗调整已通过浏览器和真实桌面验证。求职应用默认关闭，需要用户主动启用，旧数据保留。真实模型质量与 Windows 真机仍未覆盖。
