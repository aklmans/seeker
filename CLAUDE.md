...
| 3 | 求职逐页迁入 `apps/jobseek/`(每页一 commit;剩余交织按下方第8轮裁定) | **搬迁收官**(择取+抽壳序1–5 全落,index.html −71%→1330;1–22 轮全过审;8 个 `SeekerShell.*` 契约;余 3.y 类型化[单列里程碑:**步1-2 + 步3 中层叶子刀全过审(第26-32轮);剩 registry/keys/profile 卡 classic 单体 → 待「classic 单体 module 化」子阶段**]+ 归属债[settingsPersistOn 系/壳杂项/文案/CACT_ALLOWED 契约化]) |
