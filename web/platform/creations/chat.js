...
export function renderAndSaveWidget(widget,scope){
  const card=renderWidget(widget,{style:normalizeStyle()});
  ...
  function save(){
    ...
    inFlight=saveGeneratedWidget(widget,scope).then(saved=>{status.textContent=tt('已保存到我的作品','Saved to My creations');appendCreationLink(bar,saved.id);return saved.id;})
