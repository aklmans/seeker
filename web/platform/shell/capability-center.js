...
/** @param {string} id @param {string} eyebrow @param {string} title @param {string} sub */
...
export function renderCapabilityCenter() {
  ...
  host.innerHTML =
    ...
    block('cc-projects', 'WORKSPACES', tt('工作空间', 'Workspaces'), tt('按目标整理对话与助手指令。', 'Organize conversations and assistant instructions by goal.')) +
    `</div>` +
    signFoot();
  const rb = $('#ccRefresh');
  if (rb) /** @type {HTMLElement} */ (rb).onclick = () => populate(); // 手动刷新(P1-a 快照式取数;导航即刷新 = P1-b 深化时接)
  populate();
}

// 逐域异步填充:一域失败(如 web 端 notImpl)只降级本域、不拖累其他域。
/** @param {string} id @param {() => Promise<string>} fn */
...
function populate() {
  ...
  (async () => {
    ...
    if (!box) return;
    try { await renderConnectors(/** @type {HTMLElement} */ (box)); }
  ...
  fill('cc-tools', async () => {
    ...
    return caps.map((/** @type {{id: string, available?: boolean}} */ c) => {
  ...
  (async () => {
    const box = $('#cc-memory');
    if (!box) return;
    try { await renderMemory(/** @type {HTMLElement} */ (box)); }
    catch (_e) { box.innerHTML = emptyLine(tt('桌面端可用(网页端不支持此项)', 'Desktop only (unavailable on web)')); }
  })();
  (async () => {
    const box = $('#cc-docs');
    if (!box) return;
    try { await renderDocs(/** @type {HTMLElement} */ (box)); }
    catch (_e) { box.innerHTML = emptyLine(tt('桌面端可用(网页端不支持此项)', 'Desktop only (unavailable on web)')); }
  })();
  ...
  (async () => {
    ...
    if (!box) return;
    box.innerHTML='<button class="btn" data-go="workspaces">'+tt('管理工作空间','Manage workspaces')+'</button>';
  })();
}
