...
function aiStream(req, handlers = {}, startInvoke) {
  ...
  const done = (async () => {
    ...
    (startInvoke ? startInvoke(sessionId) : invoke('ai_chat', { sessionId, userText: /** @type {import('./types').AiRequest} */ (req).userText, task: req.task || null, appTools: /** @type {import('./types').AiRequest} */ (req).appTools || null, historyKey: /** @type {import('./types').AiRequest} */ (req).historyKey || null, projectInstructions: /** @type {import('./types').AiRequest} */ (req).projectInstructions || null }))
      .catch((/** @type {any} */ err) => {
        cleanup();
