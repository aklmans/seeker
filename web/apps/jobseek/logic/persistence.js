...
/** RESUME_TAILORED 从旧数据模块导出时被推断成 `{}`；在持久化边界明确真实键值形状。 @type {Record<string, any>} */
const TAILORED = RESUME_TAILORED;
...
export async function hydrateJobs(){
  ...
  try{
    const rows = /** @type {any[]} */ (await window.SeekerRT.db.list('jobs'));
...
export function persistResume(jobId){
  ...
  const r = TAILORED[jobId]; if(!r) return;
...
export function clearAllTailoredResumes(){
  const ids=Object.keys(TAILORED);
  ...
  const snap=JSON.parse(JSON.stringify(TAILORED));
  G.confirmDestructive({
    ...
    onConfirm:()=>{ for(const k of ids){ delete TAILORED[k]; removeResume(k); } renderResumes(); try{ renderInterview(); }catch(_e){} },
    onUndo:()=>{ Object.keys(snap).forEach(k=>{ TAILORED[k]=snap[k]; persistResume(k); }); renderResumes(); try{ renderInterview(); }catch(_e){} },
  });
}
export async function hydrateResumes(){
  ...
  try{
    ...
    for(const raw of rows){
      ...
      if(rec.jobId!=null) TAILORED[rec.jobId] = { template:rec.template||'minimal', modules:rec.modules||[] };
    }
