...
function renderJobs(){
  ...
  const rows=JOBS.filter(j=>(jobFilter.city==='全部'||j.city===jobFilter.city)&&(jobFilter.status==='全部'||j.status===jobFilter.status)).map(j=>{
    ...
    const chips=j.need.slice(0,3).map(n=>`<span class="chip">${n}</span>`).join('')+(j.need.length>3?`<span class="chip" style="border:none;color:var(--ink-mute);">+${j.need.length-3}</span>`:'');
    return `<tr data-job="${j.id}">
      <td class="idx">${String(j.id).padStart(2,'0')}</td>
      <td><span class="co">${j.co}</span></td>
      <td>${j.role}</td>
      <td class="mono" style="font-size:12px;color:var(--ink-3);">${j.city}</td>
      <td><span class="badge ${st.cls}">${st.label}</span></td>
      <td class="num-cell">${j.interest.toFixed(1)}<small>/10</small></td>
      <td class="num-cell">${j.match.toFixed(1)}<small>/10</small></td>
      <td class="mono" style="font-size:12px;color:var(--ink-2);">${j.pay}</td>
...
function openJobDetail(id, row){
  ...
  const need=j.need.map(n=>`<span class="chip">${n}</span>`).join(' ');
  const plus=j.plus.map(n=>`<span class="chip">${n}</span>`).join(' ');
  // build match lines: for each need skill, check user's skill level
  const matchLines=j.need.map(n=>{
    const s=skillByName(n);
    if(s&&s.lvl>=3) return `<div class="match-line"><span class="mk ok">✓</span><span class="mt"><b>${n}</b> · ${tt(s.years+' 年','y'+s.years)} · ${s.evidence[0]||tt('有实践经验','has experience')}</span></div>`;
    if(s&&s.lvl>=1) return `<div class="match-line"><span class="mk par">○</span><span class="mt">${n} · ${tt('仅基础','basic only')} · <span style="color:var(--ink-3);">${tt('可补充','to add')}</span></span></div>`;
    return `<div class="match-line"><span class="mk par">○</span><span class="mt">${n} · ${tt('学习中','learning')} · <span style="color:var(--ink-3);">${tt('可补充','to add')}</span></span></div>`;
  }).join('');
  ...
  const html=`
    <div class="modal-head">
      <div><h2>${j.co} · ${j.role}</h2>
      <div class="sub"><span>${j.city}</span><span>·</span><span class="badge ${st.cls}">${st.label}</span><span>${tt('兴趣','Interest')} ${j.interest.toFixed(1)}</span><span>·</span><span>${tt('匹配','Match')} ${j.match.toFixed(1)}</span><span>·</span><span>${j.pay}</span></div></div>
    ...
    <div class="modal-body">
      ...
      ${aiMetaHtml(j)}
      <div class="msec"><p class="seclabel">— JD ORIGINAL</p><h3 class="sectitle" style="font-size:16px;margin-bottom:12px;">${tt('完整 JD','Full JD')}<span class="dot">.</span></h3><div class="jd-text">${j.jd}</div></div>
      <div class="msec"><p class="seclabel">— EXTRACTED</p><h3 class="sectitle" style="font-size:16px;margin-bottom:12px;">${tt('抽取的要求','Extracted requirements')}<span class="dot">.</span></h3>
        ...
        <div style="display:flex;gap:32px;margin-top:16px;font-size:13px;color:var(--ink-2);"><span>${tt('年限要求','Years')} · <b class="mono">${j.years}</b></span><span>${tt('学历','Education')} · <b class="mono">${j.edu}</b></span><span>${tt('来源','Source')} · <b class="mono">${j.src}</b></span></div>
      </div>
      ...
      <div class="msec"><p class="seclabel">— SUGGESTION</p><h3 class="sectitle" style="font-size:16px;margin-bottom:8px;">${tt('可补充方向','Suggested focus')}<span class="dot">.</span></h3>
        <p style="font-size:14px;color:var(--ink);margin:0;font-weight:500;">${tt(gapSkill+' 实战经验',gapSkill+' hands-on experience')}</p>
        <p style="font-size:13px;color:var(--ink-3);margin:6px 0 12px;">${tt('可补充: 在训练项目中加入 '+gapSkill+' 的生产实践,沉淀为可展示证据。','Add '+gapSkill+' to a training project as production practice, and turn it into demonstrable evidence.')}</p>
  ...
  $('#addToActions',m).onclick=()=>{genPlanFromGap(gapSkill, j.co+' · '+j.role.split('·')[0].trim());renderActions();renderOverview();closeModal();toast('已生成训练计划: 补齐 '+gapSkill);go('actions');};
  ...
  $('#aiPlan',m).onclick=()=>{genPlanFromGap(gapSkill, j.co+' · '+j.role.split('·')[0].trim());renderActions();renderOverview();closeModal();toast('已生成「'+gapSkill+'」训练计划');go('actions');};
