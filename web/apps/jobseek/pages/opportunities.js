...
function composerHTML(servers) {
  ...
  return `<section class="sec agent-task-compose">
    ...
    <div class="field-row"><div class="field"><label>${tt('目标职位（逗号分隔）', 'Target roles (comma-separated)')}</label><input class="input" data-radar-roles placeholder="Backend Engineer, Platform Engineer"></div><div class="field"><label>${tt('地点', 'Locations')}</label><input class="input" data-radar-locations placeholder="Remote, Seattle"></div></div>
    <div class="field-row"><div class="field"><label>${tt('必备技能', 'Required skills')}</label><input class="input" data-radar-skills placeholder="Rust, SQL"></div><div class="field"><label>${tt('远程偏好', 'Remote preference')}</label><select class="select" data-radar-remote><option value="any">${tt('不限', 'Any')}</option><option value="remote">${tt('远程', 'Remote')}</option><option value="hybrid">${tt('混合', 'Hybrid')}</option><option value="onsite">${tt('现场', 'On-site')}</option></select></div></div>
    <div class="field-row"><div class="field"><label>${tt('排除关键词', 'Excluded keywords')}</label><input class="input" data-radar-excluded placeholder="unpaid, contract"></div><div class="field"><label>${tt('关注公司', 'Watched companies')}</label><input class="input" data-radar-companies placeholder="Acme"></div></div>
