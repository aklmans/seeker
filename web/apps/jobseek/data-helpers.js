...
const skillByName=(n)=>SKILLS.find(s=>s.name===n);
...
function distinctNeedSkills(){ return new Set(JOBS.flatMap(j=>j.need||[])).size; }
function distBy(field){ const m=new Map(); JOBS.forEach(j=>{ const v=(j[field]||'—'); m.set(v,(m.get(v)||0)+1); }); return [...m.entries()].sort((a,b)=>b[1]-a[1]); }
function pipelineReal(){ const by=jobsByStatus();
...
function topGapsReal(n){ // 跨岗缺口:被需求技能 → 需求岗位数 + 用户当前水平(skillByName.lvl);按需求数排序取前 n。
...
function keywordsReal(){ const text=JOBS.map(j=>j.jd||'').join('\n');
