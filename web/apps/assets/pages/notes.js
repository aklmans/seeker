...
export function renderNotes() {
  ...
  host.innerHTML = frontis('LIBRARY',tt('资料库','Library'))
    ...
    + (rows.length?rows.map(n=>`<article class="sec" data-note="${esc(n.id)}">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;"><button class="btn" data-anedit="${esc(n.id)}">${tt('编辑','Edit')}</button><button class="btn" data-mindmap="${esc(n.id)}">${tt('创建思维导图','Create mind map')}</button><button class="btn" data-export="${esc(n.id)}">${tt('导出 Markdown','Export Markdown')}</button><button class="btn" data-anai="${esc(n.id)}" ${organizing.has(n.id)||!window.SeekerRT.available('textGeneration')?'disabled':''} title="${tt('仅桌面版连接模型后可用','Available in the desktop app with a connected model')}">${organizing.has(n.id)?tt('整理中…','Organizing…'):tt('AI 整理','Organize with AI')}</button>${window.SeekerRT?.platform==='desktop'?`<button class="btn" data-knowledge="${esc(n.id)}">${tt('加入知识库','Add to knowledge')}</button>`:''}<button class="btn-text" data-andel="${esc(n.id)}">${tt('删除','Delete')}</button></div></article>`).join(''):`<div class="sec"><p>${all.length?tt('没有匹配的笔记。','No matching notes.'):tt('还没有资料，先记下一个想法吧。','Your library is empty. Start with an idea.')}</p></div>`)+signFoot();
  ...
  wire('mindmap',n=>window.SeekerShell.createFromText({text:n.text,title:noteTitle(n),source:{type:'note',id:n.id,updatedAt:n.updated}}));
