...
test('当前交互状态与长中文内容导出 PNG/PDF，快照不获得脚本和网络权限',async({page})=>{
  ...
  const pdfDownload=page.waitForEvent('download');await page.getByRole('button',{name:'保存长页 PDF',exact:true}).click();const pdf=await readFile(await(await pdfDownload).path());expect(pdf.subarray(0,8).toString()).toBe('%PDF-1.4');expect(pdf.toString('latin1')).toContain('/Subtype /Image');
