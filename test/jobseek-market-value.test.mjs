...
test('★market-value-compute.js 零 import(自包含源码守卫,第74轮 [建议]③)', () => {
  // eval 测(上)只证**被覆盖路径**的自包含(path-dependent);而顶层 import 不进 String(compute) ⇒
  // 一个只在未覆盖分支用到的 import 会逃过 eval、到真沙箱才 ReferenceError。源码守卫 path-independent:
  // 把「零 import」从注释约定变 CI 强制(镜像 registry-tools.test.mjs 的 readFileSync 源扫 + Rust prod() 守卫)。
